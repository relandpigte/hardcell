...
class pb_backupbuddy_backup {
	 ...
	 *
	 */
	public function deploy_start( $state ) {
		pb_backupbuddy::status( 'details', 'Starting deployment process. Incoming state: `' . print_r( $state, true ) . '`.' );
		
		pb_backupbuddy::status( 'startSubFunction', json_encode( array( 'function' => 'deploy_start', 'title' => 'Found deployment.' ) ) );
		
		// Send files.
		backupbuddy_core::send_remote_destination( $state['destination_id'], $this->_backup['archive_file'], 'Deployment', $send_importbuddy = false, $delete_after = true, $identifier = $this->_backup['serial'] );
		...
		if ( '0' == $fileoptions['finish_time'] ) { // Not finished yet.
			pb_backupbuddy::status( 'details', 'File send not yet finished. Started `' . ( time() - $fileoptions['finish_time'] ) . '` seconds ago. Inserting wait.' );
			$this->_backup['steps'] = array_splice( $this->_backups['step'], $this->_currentStepIndex + 1, 0, $newStep );
		...
		// App step to send plugins files that are of a newer version than destination.
		if ( true !== $state['sendPlugins'] ) {
			pb_backupbuddy::status( 'details', 'Plugin file sending skipped based on settings.' );
		} else {
			pb_backupbuddy::status( 'details', 'Insert plugins sending step here.' );
			$newStep = array(
		...
		// Add step to send active theme files that have a different crc.
		if ( true !== $state['sendTheme'] ) {
			pb_backupbuddy::status( 'details', 'Active theme file sending skipped based on settings.' );
		} else {
			pb_backupbuddy::status( 'details', 'Insert theme sending step here.' );
			$newStep = array(
		...
		// Tell destination to make an importbuddy.php in valid location.
		pb_backupbuddy::status( 'details', 'Inserting deploy step to render a remote copy of importbuddy.' );
		$newStep = array(
			'function'		=>	'deploy_renderImportBuddy',
			'args'			=>	array( $state ),
			'start_time'	=>	0,
			'finish_time'	=>	0,
			'attempts'		=>	0,
	...
	public function deploy_renderImportBuddy( $state ) {
		...
		if ( false === ( $response = backupbuddy_remote_api::remoteCall( $state['destination'], 'renderImportBuddy', array( 'backupFile' => basename( $this->_backup['archive_file'] ) ) ) ) ) {
		...
		$importbuddyURL = $state['destination']['siteurl'] . '/importbuddy-' . $response['importFileSerial'] . '.php';
		$importbuddyLogURL = $state['destination']['siteurl'] . '/importbuddy/status-' . $response['importFileSerial'] . '.txt';
		pb_backupbuddy::status( 'details', 'Load importbuddy at `' . $importbuddyURL . '` with verifier `' . $importbuddyPassword . '`.' );
		pb_backupbuddy::status( 'loadImportBuddy', json_encode( array( 'url' => $importbuddyURL . '?ajax=2&v=' . $importbuddyPassword . '&deploy=true&file=' . basename( $this->_backup['archive_file'] ), 'logurl' => $importbuddyLogURL ) ) );
	...
	public function ms_download_extract_wordpress() {
		...
		pb_backupbuddy::status( 'details', 'Downloading latest WordPress ZIP file.' );
