...
vars.next.css({top:values.nextTop,left:values.nextLeft}).animate({top:0,left:0,opacity:1},settings.effectTime,values.nextEasing,function(){callback($slider,settings);});},explode:function($slider,params,callback){var
...
explode($slider,settings);}};$.fn.rhinoslider.preparations={fade:function($slider,settings,vars){},slide:function($slider,settings,vars){vars.items.css('overflow','hidden');$slider.css('overflow','hidden');},explode:function($slider,settings,vars){var shiftValue=String(tmpShiftValue);if(shiftValue.indexOf(',')>=0){var tmp=shiftValue.split(',');settings.shiftValue.x=tmp[0];settings.shiftValue.y=tmp[1];}else{settings.shiftValue.x=parseInt(tmpShiftValue,10);settings.shiftValue.y=parseInt(tmpShiftValue,10);}
